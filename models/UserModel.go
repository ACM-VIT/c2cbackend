...
type User struct {
	...
	ProfilePictureURL string   `gorm:"type:text" json:"profile_picture_url" valid:"url~URL is not valid,optional"`
	...
	Gender            string   `gorm:"type:varchar(10);not null" json:"gender" valid:"in(male|female|other)~Gender must be male female or other,optional"`
	RegNo             string   `gorm:"type:varchar(20);not null;uniqueIndex" json:"reg_no" valid:"required~Registration number is required"`
	Role              UserRole `gorm:"type:varchar(20);not null" json:"role" valid:"in(admin|reviewer|participant)~Role must be admin/reviewer/participant,optional"`
	TeamID            string   `gorm:"type:varchar(36);default:null" json:"team_id"`
